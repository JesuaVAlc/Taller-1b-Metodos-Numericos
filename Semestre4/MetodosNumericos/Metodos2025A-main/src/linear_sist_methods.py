...
def eliminacion_gaussiana(A: np.ndarray) -> np.ndarray:
    ...
    for i in range(0, n - 1):  # loop por columna
        ...
        for j in range(i + 1, n):
            m = A[j, i] / A[i, i]
            multiplicaciones_divisiones+=1
            A[j, i:] = A[j, i:] - m * A[i, i:]
            multiplicaciones_divisiones+= (n-i)
            sumas_restas+=(n-i)
    ...
    #logging.info(f"Numero de intercambios de filas hechos: {n_intercambios}")
   # logging.info(f"Multiplicaciones/Divisiones: {multiplicaciones_divisiones}")
    #logging.info(f"Sumas/Restas: {sumas_restas}")
    return solucion
...
def gauss_jordan(Ab: np.ndarray) -> np.ndarray:
    ...
    for i in range(0, n):  # loop por columna
        ...
        for j in range(n):
            ...
            multiplicaciones_divisiones += (Ab.shape[1]-(i+1))
            sumas_restas+=(Ab.shape[1]-(i+1))
